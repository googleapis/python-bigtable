...
class _State(Enum):
    NEW_ROW = "NEW_ROW"
    ROW_IN_PROGRESS = "ROW_IN_PROGRESS"
    ...
    def process_chunks(self, response):
        ...
        for chunk in response.chunks:
            ...
            if self.state == _State.NEW_ROW:
                ...
                self.state = _State.ROW_IN_PROGRESS

            if self.state == _State.ROW_IN_PROGRESS:
                ...
                if self.row.cell.family is None:
                ...
                if chunk.value_size == 0:
                    ...
                    self.row.cell.value = bytearray(chunk.value_size)
            ...
            if self.state == _State.CELL_IN_PROGRESS:
                ...
                s = slice(
                    self.row.cell.value_index,
                    self.row.cell.value_index + len(chunk.value),
                )
                self.row.cell.value[s] = chunk.value
                self.row.cell.value_index += len(chunk.value)
            ...
            if self.state == _State.ROW_IN_PROGRESS:
                ...
                if chunk.commit_row:
                    raise InvalidChunk("chunk tried to commit an in progress row")

            if self.state == _State.ROW_COMPLETE:
                self.last_seen_row_key = self.row.row_key
                row = PartialRowData(self.row.row_key)
                row._cells = self.row.cells
                yield row
                self.row = None
                self.state = _State.NEW_ROW

    def finalize(self):
        if self.row or self.state != _State.NEW_ROW:
