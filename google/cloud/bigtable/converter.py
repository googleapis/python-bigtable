...
class AsyncToSync(ast.NodeTransformer):
    ...
    def visit_Subscript(self, node):
        if hasattr(node, "value") and isinstance(node.value, ast.Name) and node.value.id == "AsyncGenerator" and name_map.get(node.value.id, "") == "Generator":
            # Generator has different argument signature than AsyncGenerator
            return ast.copy_location(
                ast.Subscript(
                    ast.Name("Generator"),
                    ast.Index(ast.Tuple([self.visit(i) for i in node.slice.elts + [ast.Constant("Any")]])),
                    node.ctx,
                ),
                node,
            )
        elif hasattr(node, "value") and isinstance(node.value, ast.Name) and name_map.get(node.value.id, False) is None:
...
if __name__ == "__main__":
    ...
    for cls in conversion_list:
        new_tree, new_imports = transform_sync(cls, skip_methods=["mutations_batcher"])
        if tree is None:
            tree = new_tree
        else:
