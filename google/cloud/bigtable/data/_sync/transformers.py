...
class CrossSyncFileArtifact:
    ...
    """
    file_path: str
    imports: list[ast.Import | ast.ImportFrom | ast.Try | ast.If] = field(default_factory=list)
    ...
    def render(self, with_black=True, save_to_disk=False) -> str:
        ...
        if with_black:
            import black
            import autoflake
            full_str = black.format_str(autoflake.fix_code(full_str, remove_all_unused_imports=True), mode=black.FileMode())
        if save_to_disk:
...
class CrossSyncClassParser(ast.NodeTransformer):
    ...
    def __init__(self, file_path):
        ...
        self.cross_sync_converter = SymbolReplacer({"CrossSync": "CrossSync._Sync_Impl"})


    def convert_file(self, artifacts:set[CrossSyncFileArtifact]|None=None) -> set[CrossSyncFileArtifact]:
        """
    ...
    def visit_ClassDef(self, node):
        ...
        for decorator in node.decorator_list:
            if "CrossSync" in ast.dump(decorator):
                ...
                output_artifact = self._artifact_dict.get(out_file, CrossSyncFileArtifact(out_file))
                ...
                if sync_cls_name not in output_artifact.contained_classes:
                    ...
                    mypy_ignores = [s for s in kwargs.get("mypy_ignore", []) if s not in output_artifact.mypy_ignore]
                    output_artifact.mypy_ignore.extend(mypy_ignores)
                    # handle file-level imports
                    if not output_artifact.imports and kwargs.get("include_file_imports", True):
                        output_artifact.imports = self.imports
                self._artifact_dict[out_file] = output_artifact
        return node

    def _transform_class(self, cls_ast: ast.ClassDef, new_name:str, replace_symbols=None, **kwargs) -> ast.ClassDef:
        """
        ...
        if hasattr(cls_ast, "decorator_list"):
            cls_ast.decorator_list = [d for d in cls_ast.decorator_list if "CrossSync" not in ast.dump(d)]
    ...
    def _get_imports(self, tree:ast.Module) -> list[ast.Import | ast.ImportFrom | ast.Try | ast.If]:
        """
    ...
    def _convert_ast_to_py(self, ast_node):
        ...
        if isinstance(ast_node, ast.Dict):
            return {self._convert_ast_to_py(k): self._convert_ast_to_py(v) for k, v in zip(ast_node.keys, ast_node.values)}
