...
class _ReadRowsOperation_SyncAutoGenerated(Iterable[Row]):
    ...
    @staticmethod
    def _generator_to_buffer(
        buffer: queue.Queue[Any], input_generator: Iterable[Any]
    ) -> None:
        ...
        try:
            for item in input_generator:
                buffer.put(item)
                time.sleep(0)
            buffer.put(StopIteration)
        except Exception as e:
            buffer.put(e)

    @staticmethod
    def _buffer_to_generator(buffer: queue.Queue[Any]) -> Generator[Any, None, "Any"]:
...
class Table_SyncAutoGenerated:
    ...
    def __init__(
        self,
        client: BigtableDataClient_SyncAutoGenerated,
        instance_id: str,
        table_id: str,
    ...
    def read_rows_stream(
        ...
        *,
        ...
        operation_timeout: float | None = None,
        per_request_timeout: float | None = None,
    ) -> ReadRowsIterator_SyncAutoGenerated:
        ...
        row_merger = _ReadRowsOperation_SyncAutoGenerated(
            request,
        ...
        output_generator = ReadRowsIterator_SyncAutoGenerated(row_merger)
    ...
    def mutations_batcher(self, **kwargs) -> MutationsBatcher:
        raise NotImplementedError("Method generation intentionally skipped")
    ...
    def close(self):
        """Called to close the Table_SyncAutoGenerated instance and release any resources held by it."""
...
class BigtableDataClient_SyncAutoGenerated(ClientWithProject):
    ...
    def get_table(
        ...
        return Table_SyncAutoGenerated(
            self,
            instance_id,
            table_id,
            app_profile_id,
...
class ReadRowsIterator_SyncAutoGenerated(Iterable[Row]):
    ...
    def __init__(self, merger: _ReadRowsOperation_SyncAutoGenerated):
        self._merger: _ReadRowsOperation_SyncAutoGenerated = merger
    ...
    def _idle_timeout_coroutine(self, idle_timeout: float):
        """Coroutine that will cancel a stream if no interaction with the iterator in the last `idle_timeout` seconds."""
        while self.active:
            next_timeout = self.last_interaction_time + idle_timeout
            time.sleep(next_timeout - time.time())
            if self.last_interaction_time + idle_timeout < time.time() and self.active:
                self._finish_with_error(
                    IdleTimeout(
                        f"Timed out waiting for next Row to be consumed. (idle_timeout={idle_timeout:0.1f}s)"
                    )
                )
    ...
    def __next__(self) -> Row:
        """Implement the async iterator potocol. Return the next item in the stream if active, or raise an exception if the stream has been closed."""
