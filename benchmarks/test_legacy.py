...
def test_legacy_scan_throughput_benchmark(populated_table, legacy_table, scan_size, duration=TEST_DURATION):
    """
    ...
    """
    print(f"\nrunning test_legacy_scan_throughput_benchmark for {duration}s with scan_size={scan_size}")
    total_rows = 0
    total_operations = 0
    total_op_time = 0
    while total_op_time < duration:
        ...
        for row in legacy_table.read_rows(start_key=start_key, limit=scan_size):
            total_rows += 1
...
def test_legacy_point_read_throughput_benchmark(populated_table, legacy_table, duration=TEST_DURATION):
    """
    ...
    """
    print(f"\nrunning test_legacy_point_read_throughput for {duration}s")
    total_rows = 0
    total_op_time = 0
    while total_op_time < duration:
        ...
        start_time = time.perf_counter()
        legacy_table.read_row(chosen_key)
        total_op_time += time.perf_counter() - start_time
        total_rows += 1
