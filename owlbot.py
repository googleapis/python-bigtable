...
for library in get_staging_dirs(bigtable_admin_default_version, "bigtable_admin"):
    is_fresh_admin_v2_copy = \
        s.move(library / "google/cloud/bigtable/admin_v2",
               destination="google/cloud/bigtable/admin",
               excludes=["**/gapic_version.py"])
    s.move(library / "tests/unit/gapic/admin_v2", destination="tests/unit/gapic/admin")
...
# Add overlay imports to top level __init__.py files in admin at the end
...
# Because we are moving google/bigtable/admin_v2 in googleapis to google/cloud/bigtable/admin,
# and because of selective GAPIC, we need to modify the samples in the autogenerated samples and
# docstrings. We will fix samples by:
#   1. Replacing all instances of admin_v2 with admin in the samples, code, and autogen doc pipeline.
#   2. Replacing all instances of BaseBigtableTableAdminClient/BaseBigtableAdminAsyncClient with
#      BigtableTableAdminClient/BigtableTableAdminAsyncClient.
s.replace(
    [
        "google/cloud/bigtable/admin/services/**/*.py",
        "google/cloud/bigtable/admin/types/*.py",
        "google/cloud/bigtable/admin/__init__.py",
        "docs/admin_client/*.rst",
        "tests/unit/gapic/admin/test_*.py",
    ],
    "admin_v2",
    "admin"
)

# Need to be more careful replacing in samples files to not alter region tag names.
s.replace(
    [
...
s.replace(
    [
        "google/cloud/bigtable/admin/gapic_metadata.json",
    ],
    "\"libraryPackage\": \"google.cloud.bigtable.admin_v2\"",
    "\"libraryPackage\": \"google.cloud.bigtable.admin\""
)

s.replace(
    [
        "google/cloud/bigtable/admin/services/*/client.py",
        "google/cloud/bigtable/admin/services/*/async_client.py",
        "samples/generated_samples/bigtableadmin_v2_*.py"
    ],
    r"client = admin\.Base(BigtableTableAdmin(Async)?Client\(\))",
    r"client = admin.\1"
)

# Fix an improperly formatted table that breaks nox -s docs.
s.replace(
    "google/cloud/bigtable/admin/types/table.py",
    """            For example, if \\\\_key =
...
if is_fresh_admin_docs_copy:
    ...
    s.replace(
        ...
        r"^\.\. automodule:: google\.cloud\.bigtable\.admin\.services\.bigtable_table_admin$",
    ...
    s.replace(
        ...
        r"""(\.\. automodule:: google\.cloud\.bigtable\.admin\.types
    :members:
    :show-inheritance:)
""",
        r"""\1
...
if is_fresh_admin_v2_copy:
    ...
    s.replace(
        "google/cloud/bigtable/admin/types/table.py",
        r"^(from google\.cloud\.bigtable\.admin\.types import .+)$",
        r"""\1
