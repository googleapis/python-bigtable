...
def test_bigtable_write_read_drop_truncate():
    ...
    for i, row_key in enumerate(row_keys):
        ...
        row.set_cell(
            COLUMN_FAMILY_ID, col_name, value, timestamp=datetime.datetime.now(datetime.timezone.utc)
        )
        rows.append(row)
...
def test_bigtable_mutations_batcher():
    ...
    row = table.row(row_key)
    row.set_cell(
        COLUMN_FAMILY_ID, column_name, "value-0", timestamp=datetime.datetime.now(datetime.timezone.utc)
    )
    batcher.mutate(row)
    ...
    for i in range(1, len(row_keys)):
        ...
        value = "value_{}".format(i).encode()
        row.set_cell(
            COLUMN_FAMILY_ID, column_name, value, timestamp=datetime.datetime.now(datetime.timezone.utc)
        )
        rows.append(row)
...
def test_bigtable_batcher_mutate_flush_mutate_rows():
    ...
    row = table.row(row_key)
    row.set_cell(
        COLUMN_FAMILY_ID, COL_NAME1, "value-0", timestamp=datetime.datetime.now(datetime.timezone.utc)
    )
...
def test_bigtable_row_data_cells_cell_value_cell_values():
    ...
    row.set_cell(
        COLUMN_FAMILY_ID, COL_NAME1, value, timestamp=datetime.datetime.now(datetime.timezone.utc)
    )
    row.commit()
...
def test_bigtable_row_setcell_rowkey():
    ...
    cell_val = b"cell-val"
    row.set_cell(
        COLUMN_FAMILY_ID, COL_NAME1, cell_val, timestamp=datetime.datetime.now(datetime.timezone.utc)
    )
