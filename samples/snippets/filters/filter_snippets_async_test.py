      ...
      async with table.mutations_batcher() as batcher:
        await batcher.append(
          RowMutationEntry(
            ...
            [
              SetCell("stats_summary", "connected_cell", 1,
                      _microseconds_from_datetime(timestamp)),
              SetCell("stats_summary", "connected_cell", 1,
                      _microseconds_from_datetime(timestamp)),
              SetCell("stats_summary", "connected_wifi", 1,
                      _microseconds_from_datetime(timestamp)),
              SetCell("stats_summary", "os_build", "PQ2A.190405.003",
                      _microseconds_from_datetime(timestamp)),
              SetCell("cell_plan", "data_plan_01gb", "true",
                      _microseconds_from_datetime(timestamp_minus_hr)),
              SetCell("cell_plan", "data_plan_01gb", "false",
                      _microseconds_from_datetime(timestamp)),
              SetCell("cell_plan", "data_plan_05gb", "true",
                      _microseconds_from_datetime(timestamp)),
            ]))
        await batcher.append(
          RowMutationEntry(
            ...
            [
              ...
              SetCell("stats_summary", "connected_wifi", 1,
                      _microseconds_from_datetime(timestamp)),
              SetCell("stats_summary", "os_build", "PQ2A.190405.004",
                      _microseconds_from_datetime(timestamp)),
              SetCell("cell_plan", "data_plan_05gb", "true",
                      _microseconds_from_datetime(timestamp)),
            ]
        ...
        await batcher.append(
          RowMutationEntry(
            ...
            [
              SetCell("stats_summary", "connected_cell", 0,
                      _microseconds_from_datetime(timestamp)),
              SetCell("stats_summary", "connected_wifi", 1,
                      _microseconds_from_datetime(timestamp)),
              SetCell("stats_summary", "os_build", "PQ2A.190406.000",
                      _microseconds_from_datetime(timestamp)),
              SetCell("cell_plan", "data_plan_05gb", "true",
                      _microseconds_from_datetime(timestamp)),
            ]
        ...
        await batcher.append(
          RowMutationEntry(
            ...
            [
              ...
              SetCell("stats_summary", "connected_wifi", 1,
                      _microseconds_from_datetime(timestamp)),
              SetCell("stats_summary", "os_build", "PQ2A.190401.002",
                      _microseconds_from_datetime(timestamp)),
              SetCell("cell_plan", "data_plan_10gb", "true",
                      _microseconds_from_datetime(timestamp)),
            ]
          ))
        await batcher.append(
          RowMutationEntry(
            ...
            [
              SetCell("stats_summary", "connected_cell", 1,
                      _microseconds_from_datetime(timestamp)),
              SetCell("stats_summary", "connected_wifi", 0,
                      _microseconds_from_datetime(timestamp)),
              SetCell("stats_summary", "os_build", "PQ2A.190406.000",
                      _microseconds_from_datetime(timestamp)),
              SetCell("cell_plan", "data_plan_10gb", "true",
...
async def test_filter_limit_row_sample(capsys, table_id):
  await filter_snippets_async.filter_limit_row_sample(PROJECT,
                                                      BIGTABLE_INSTANCE,
                                                      table_id)
...
async def test_filter_limit_row_regex(capsys, table_id):
  await filter_snippets_async.filter_limit_row_regex(PROJECT, BIGTABLE_INSTANCE,
                                                     table_id)
...
async def test_filter_limit_cells_per_col(capsys, table_id):
  await filter_snippets_async.filter_limit_cells_per_col(PROJECT,
                                                         BIGTABLE_INSTANCE,
                                                         table_id)
...
async def test_filter_limit_cells_per_row(capsys, table_id):
  await filter_snippets_async.filter_limit_cells_per_row(PROJECT,
                                                         BIGTABLE_INSTANCE,
                                                         table_id)
...
async def test_filter_limit_col_family_regex(capsys, table_id):
  await filter_snippets_async.filter_limit_col_family_regex(PROJECT,
                                                            BIGTABLE_INSTANCE,
                                                            table_id)
...
async def test_filter_limit_col_range(capsys, table_id):
  await filter_snippets_async.filter_limit_col_range(PROJECT, BIGTABLE_INSTANCE,
                                                     table_id)
...
async def test_filter_limit_value_range(capsys, table_id):
  await filter_snippets_async.filter_limit_value_range(PROJECT,
                                                       BIGTABLE_INSTANCE,
                                                       table_id)
...
async def test_filter_limit_value_regex(capsys, table_id):
  await filter_snippets_async.filter_limit_value_regex(PROJECT,
                                                       BIGTABLE_INSTANCE,
                                                       table_id)
...
async def test_filter_limit_timestamp_range(capsys, table_id):
  await filter_snippets_async.filter_limit_timestamp_range(PROJECT,
                                                           BIGTABLE_INSTANCE,
                                                           table_id)
...
async def test_filter_limit_block_all(capsys, table_id):
  await filter_snippets_async.filter_limit_block_all(PROJECT, BIGTABLE_INSTANCE,
                                                     table_id)
...
async def test_filter_limit_pass_all(capsys, table_id):
  await filter_snippets_async.filter_limit_pass_all(PROJECT, BIGTABLE_INSTANCE,
                                                    table_id)
...
async def test_filter_modify_strip_value(capsys, table_id):
  await filter_snippets_async.filter_modify_strip_value(PROJECT,
                                                        BIGTABLE_INSTANCE,
                                                        table_id)
...
async def test_filter_modify_apply_label(capsys, table_id):
  await filter_snippets_async.filter_modify_apply_label(PROJECT,
                                                        BIGTABLE_INSTANCE,
                                                        table_id)
...
async def test_filter_composing_chain(capsys, table_id):
  await filter_snippets_async.filter_composing_chain(PROJECT, BIGTABLE_INSTANCE,
                                                     table_id)
...
async def test_filter_composing_interleave(capsys, table_id):
  await filter_snippets_async.filter_composing_interleave(PROJECT,
                                                          BIGTABLE_INSTANCE,
                                                          table_id)
...
async def test_filter_composing_condition(capsys, table_id):
  await filter_snippets_async.filter_composing_condition(PROJECT,
                                                         BIGTABLE_INSTANCE,
                                                         table_id)
