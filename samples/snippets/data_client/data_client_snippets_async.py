...
async def bulk_mutate(table):
    ...
    common_mutation = SetCell(
        family="family", qualifier="qualifier", new_value="value"
    )
    await table.bulk_mutate([
...
async def mutations_batcher(table):
    ...
    async with table.mutations_batcher(
        flush_count=2, max_row_bytes=1024
    ) as batcher:
        for i in range(10):
            row_key = f"row-{i}"
...
async def read_rows_list(table):
    ...
    query = ReadRowsQuery(row_ranges=[RowRange("start", "end")])
...
async def read_modify_write_increment(table):
    ...
    add_two_rule = IncrementRule(family, qualifier, increment=2)
...
async def read_modify_write_append(table):
    ...
    append_world_rule = AppendValueRule(family, qualifier, value=" world")
