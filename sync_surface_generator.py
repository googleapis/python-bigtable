...
if __name__ == "__main__":
    ...
    for output_file in class_map.keys():
        ...
        for async_class in class_map[output_file]:
            ...
            file_mypy_ignore.update(async_class.cross_sync_mypy_ignore)
        ...
        mypy_ignore_str = ", ".join(file_mypy_ignore)
        if mypy_ignore_str:
            mypy_ignore_str = f"# mypy: disable-error-code=\"{mypy_ignore_str}\""
        # append clean tree
        ...
        """
        full_code = f"{header}{mypy_ignore_str}\n\n{import_str}\n\n{ast.unparse(combined_tree)}"
