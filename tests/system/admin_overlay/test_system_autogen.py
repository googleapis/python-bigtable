...
def table_admin_client(admin_overlay_project_id):
    with admin.BigtableTableAdminClient(
...
def create_instance(
    ...
    if os.getenv(BIGTABLE_EMULATOR):
        ...
        create_instance_request = admin.CreateInstanceRequest(
            ...
            instance=admin.Instance(display_name=instance_id[:30]),
            clusters=clusters,
        )
    ...
    create_table_request = admin.CreateTableRequest(
        ...
        table=admin.Table(
            column_families={TEST_COLUMMN_FAMILY_NAME: admin.ColumnFamily()}
        ),
    )
...
@pytest.mark.parametrize(
    ...
    [(admin.StorageType.HDD, False), (admin.StorageType.SSD, True)],
)
def test_optimize_restored_table(
    ...
    assert isinstance(restore_operation, admin.RestoreTableOperation)
...
def test_wait_for_consistency(
    ...
    wait_for_consistency_request = admin.WaitForConsistencyRequest(
        name=table.name, standard_read_remote_writes=admin.StandardReadRemoteWrites()
    )
