...
class TestBigtableClient(object):
    ...
    def test_mutate_row(self):
        ...
        assert len(channel.requests) == 1
        expected_request = bigtable_pb2.MutateRowRequest(table_name=table_name, row_key=row_key, mutations=mutations)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_mutate_rows(self):
        ...
        assert len(channel.requests) == 1
        expected_request = bigtable_pb2.MutateRowsRequest(table_name=table_name, entries=entries)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_check_and_mutate_row(self):
        ...
        assert len(channel.requests) == 1
        expected_request = bigtable_pb2.CheckAndMutateRowRequest(table_name=table_name, row_key=row_key)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
    ...
    def test_read_modify_write_row(self):
        ...
        assert len(channel.requests) == 1
        expected_request = bigtable_pb2.ReadModifyWriteRowRequest(table_name=table_name, row_key=row_key, rules=rules)
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request
