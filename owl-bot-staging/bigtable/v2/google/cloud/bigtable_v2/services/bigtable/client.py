...
class BigtableClient(metaclass=BigtableClientMeta):
    ...
    def _get_api_endpoint(api_override, client_cert_source, universe_domain, use_mtls_endpoint):
        ...
        if api_override is not None:
            ...
            api_endpoint = BigtableClient._DEFAULT_ENDPOINT_TEMPLATE.format(UNIVERSE_DOMAIN=universe_domain)
        return api_endpoint
    ...
    def __init__(self, *,
        ...
        self._use_client_cert, self._use_mtls_endpoint, self._universe_domain_env = BigtableClient._read_environment_variables()
        self._client_cert_source = BigtableClient._get_client_cert_source(self._client_options.client_cert_source, self._use_client_cert)
        self._universe_domain = BigtableClient._get_universe_domain(universe_domain_opt, self._universe_domain_env)
        ...
        if not transport_provided:
            ...
            transport_init: Union[Type[BigtableTransport], Callable[..., BigtableTransport]] = (
    ...
    def read_rows(self,
        ...
        routing_param_regex = re.compile('^(?P<table_name>projects/[^/]+/instances/[^/]+/tables/[^/]+)$')
        ...
        routing_param_regex = re.compile('^(?P<authorized_view_name>projects/[^/]+/instances/[^/]+/tables/[^/]+/authorizedViews/[^/]+)$')
    ...
    def check_and_mutate_row(self,
        ...
        # - Quick check: If we got a request object, we should *not* have
        #   gotten any keyword arguments that map to the request.
        flattened_params = [table_name, row_key, predicate_filter, true_mutations, false_mutations, app_profile_id]
        has_flattened_params = len([param for param in flattened_params if param is not None]) > 0
        if request is not None and has_flattened_params:
            raise ValueError('If the `request` argument is set, then none of '
                             'the individual field arguments should be set.')
    ...
    def generate_initial_change_stream_partitions(self,
            request: Optional[Union[bigtable.GenerateInitialChangeStreamPartitionsRequest, dict]] = None,
            *,
            table_name: Optional[str] = None,
            app_profile_id: Optional[str] = None,
            retry: OptionalRetry = gapic_v1.method.DEFAULT,
        ...
        # Wrap the RPC method; this adds retry and timeout information,
        # and friendly error handling.
        rpc = self._transport._wrapped_methods[self._transport.generate_initial_change_stream_partitions]
