...
def test_generate_initial_change_stream_partitions_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = bigtable.GenerateInitialChangeStreamPartitionsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
...
def test_read_change_stream_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        return_value = bigtable.ReadChangeStreamResponse(
              data_change=bigtable.ReadChangeStreamResponse.DataChange(type_=bigtable.ReadChangeStreamResponse.DataChange.Type.USER),
        )
