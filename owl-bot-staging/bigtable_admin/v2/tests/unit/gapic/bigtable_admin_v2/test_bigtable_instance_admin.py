...
def test_get_cluster(request_type, transport: str = 'grpc'):
    ...
    with mock.patch.object(
        ...
        call.return_value = instance.Cluster(
            ...
            cluster_config=instance.Cluster.ClusterConfig(cluster_autoscaling_config=instance.Cluster.ClusterAutoscalingConfig(autoscaling_limits=instance.AutoscalingLimits(min_serve_nodes=1600))),
        )
...
async def test_list_app_profiles_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_app_profiles(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_list_hot_tablets_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_hot_tablets(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_create_app_profile_rest(request_type):
    ...
    # send a request that will satisfy transcoding
    request_init = {'parent': 'projects/sample1/instances/sample2'}
    request_init["app_profile"] = {'name': 'name_value', 'etag': 'etag_value', 'description': 'description_value', 'multi_cluster_routing_use_any': {'cluster_ids': ['cluster_ids_value1', 'cluster_ids_value2']}, 'single_cluster_routing': {'cluster_id': 'cluster_id_value', 'allow_transactional_writes': True}}
    request = request_type(**request_init)
...
def test_list_app_profiles_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = bigtable_instance_admin.ListAppProfilesResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
...
def test_update_app_profile_rest(request_type):
    ...
    request_init["app_profile"] = {'name': 'projects/sample1/instances/sample2/appProfiles/sample3', 'etag': 'etag_value', 'description': 'description_value', 'multi_cluster_routing_use_any': {'cluster_ids': ['cluster_ids_value1', 'cluster_ids_value2']}, 'single_cluster_routing': {'cluster_id': 'cluster_id_value', 'allow_transactional_writes': True}}
    request = request_type(**request_init)
...
def test_list_hot_tablets_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = bigtable_instance_admin.ListHotTabletsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
