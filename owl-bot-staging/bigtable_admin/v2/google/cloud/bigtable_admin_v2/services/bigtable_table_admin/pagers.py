...
class ListSchemaBundlesAsyncPager:
    ...
    """
    def __init__(self,
            method: Callable[..., Awaitable[bigtable_table_admin.ListSchemaBundlesResponse]],
            request: bigtable_table_admin.ListSchemaBundlesRequest,
            response: bigtable_table_admin.ListSchemaBundlesResponse,
            *,
            retry: OptionalAsyncRetry = gapic_v1.method.DEFAULT,
    ...
    @property
    async def pages(self) -> AsyncIterator[bigtable_table_admin.ListSchemaBundlesResponse]:
        yield self._response
        while self._response.next_page_token:
            self._request.page_token = self._response.next_page_token
            self._response = await self._method(self._request, retry=self._retry, timeout=self._timeout, metadata=self._metadata)
